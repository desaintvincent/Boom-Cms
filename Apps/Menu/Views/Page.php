<div class="select-field">
    <div class='sub-title'><?=__('Page')?> :</div>
    <select class="form-control js-data-example-ajax required" data-url="<?=BASE_URL?>app/Menu/Menus/ajax/" id="addInputArg"></select>
</div>
<div class="text-field">
    <div class='sub-title'><?=__('Titre')?> :</div>
    <input type="text" class="form-control required" id="addInputTitle">
</div>
<div class="actions-field">
    <button class="btn btn-success" id="addButton"><?=('Ajouter')?></button>
    <button class="btn btn-warning" id="cancelButton"><?=('Annuler')?></button>
</div>
